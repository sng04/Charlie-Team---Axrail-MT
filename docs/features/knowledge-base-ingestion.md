...
> **Note:** Per-project S3 bucket routing depends on the Projects table CRUD being completed. Once projects have their own S3 buckets, S3 event notifications should be configured per-bucket. Until then, all files go to the shared KB bucket with the `{project_id}/` key prefix convention.
