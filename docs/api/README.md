...
Response includes `agent_id` and `agent_name` fields when an agent is assigned.
...
Accepts `agent_id` to update the assigned agent for the project.
...
The response includes a `suggested_questions` array when the project has previous sessions.
...
Manage knowledge base documents for a project. See [Knowledge Base Ingestion](../features/knowledge-base-ingestion.md) for the ingestion pipeline details.
...
Returns all KB documents for the project, including user uploads and auto-registered meeting summaries.

### Create KB Document

```
POST /projects/{projectId}/kb-documents
```
...
Returns an `upload_url` (pre-signed S3 URL) for direct file upload. A `KbDocuments` record is created with `status: "pending"` — it transitions to `"active"` after ingestion completes.
...
```
POST /projects/{projectId}/kb-documents/{documentId}/replace
```

Replaces an existing KB document. Returns a new `upload_url` for the replacement file.
...
```
GET /files/download?key={s3_key}&bucket={bucket}
```

Generates a pre-signed S3 download URL. The `bucket` parameter defaults to the KB bucket. Pass the skills bucket name for skill files.
...
Returns the meeting summary markdown generated by `endMeeting`. Returns `404` if the summary has not yet been generated.
...
Returns auto-generated suggested questions for a session.
